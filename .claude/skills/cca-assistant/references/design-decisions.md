...
### Unknown tilt resolves as tilted, unknown open withholds via lockout_now only (opt-in)

`limit_lowering_on_unknown_contact_state` (`auto_ventilate_options`) makes an unknown/unavailable **tilted** contact resolve as `'tlt'` instead of `'cls'`. Resolved in `trigger_variables`, like `shading_over_ventilation`, because `effective_state` needs it before `prevent_flags` exists.

Consumed at 5 sites: `effective_state`'s `w`, `recovered_window`, `window_tilted_now`, both `contact_missing` gates. The unreadable-check (`tilted_invalid`, expand()-aware for group dropout) is a shared top-level `variables:` key for the first three; the trigger `enabled:` and global `conditions:` need their own copy (Invariant 10, no shared scope).

**Tilted rationale:** VENT is safe under both truths: a still-closed window loses a bit of shading, an actually-open one is exactly where it should be.

**Opened contact gets a narrower extension**, combined inline at its exactly two consumers (`lockout_now.*` and `shading_end_state`'s `'lock'` branch) as `window_opened_now or (limit_lowering_on_unknown_contact_state and opened_invalid)`. Deliberately not factored into a shared `window_opened_now_or_unknown` name: unlike `window_tilted_now` (10 pre-existing consumers all wanting the identical widened value), these two consumers are both new code, and every prior bug in this feature was the same shape, the toggle's effect decided once upstream, invisible at the consumer that actually needed to know. Combining the raw fact (`opened_invalid`) with the toggle right at each decision point keeps that visible. Only the *passive* half (withhold a close/shading) is safe to widen this way, since a wrongly-assumed-open window driving the cover further open is not safe. The *proactive* half (`window_opened_now` itself, and every other consumer: contact handler's own drive, LOCKOUT's `s_opn`, resident target chains, `recovery_target`) stays confirmed-only. Do not extend `window_opened_now` beyond the two named consumers.

**`shading_end_state`'s `'lock'` is not a pure state sync like `lockout_now.*`**: it can reach a real `drive_plan` via the "Move cover after shading end" fallback (taken when the dedicated lockout branch's position gate doesn't hold), which also bypasses `manual_allows_event.shd` for `'lock'`. Its `will_drive` carries `not (shading_end_state == 'lock' and not window_opened_now)`: a no-op for a confirmed reading, a hard block for the unconfirmed one regardless of manual-override state. This guard reads `shading_end_state`'s already-decided output rather than re-combining `opened_invalid`/the toggle itself, since `'lock'` can only ever come from the one `elif` above (force values are `non/opn/cls/shd/vnt`, never `'lock'`), so re-deriving there would duplicate a decision already made, not add visibility. Any future drive-capable consumer of `'lock'` needs the same guard.

**`contact_missing` stays unrelaxed for the opened contact** (unlike tilted), but is now expand()-aware so a masked group-member dropout can't slip past it either. `opened_invalid` is a shared top-level `variables:` key (same role as `tilted_invalid`) for `lockout_now`/`shading_end_state`; the two `contact_missing` sites need their own copy (Invariant 10). The one case the gate still blocks (last known open, now unreadable) is load-bearing: `leave_target`'s default chain has no lockout of its own and reads confirmed-only `window_opened_now`; freezing that case is what makes the narrower `lockout_now` extension safe elsewhere.

`win_on_lockout` (shared dict beside `lockout_now`) asserts `win: 'opn'` only when confirmed, else preserves `helper_state_window`, never a confirmation that never happened. Do not simplify back to a literal `win: 'opn'`.

**The closing vent-floor branch does not gate on `lockout_tilted_when_closing`, and must not.** "Window tilted. No lockout. Move to ventilation position instead of closing" used to carry `not lockout_tilted_when_closing` in its conditions, a leftover from before this feature existed, when `window_tilted_now` and `window_tilted_confirmed` were the same thing and the guard was harmless: a confirmed-tilted reading with the option on was already fully claimed by the earlier "Lockout protection when closing" branch (`lockout_now.closing`, gated on `window_tilted_confirmed`), which ends in `stop:`, so the guard was never doing exclusion work choose-ordering wasn't already doing. Once `window_tilted_now` could be true from an *unconfirmed* reading too, the stale guard started excluding a case the lockout branch never claims (`lockout_now.closing` needs `window_tilted_confirmed`, false here). With `lockout_tilted_closing` on and the reading merely unknown, neither branch fired and execution fell through to a full close, worse than either protection, and the opposite of what the toggle promises. Its two siblings, "Shading start - hold ventilation floor (window tilted)" and "Ventilation after shading ends", never carried this guard and never had the bug; closing now matches them. Do not re-add a `lockout_tilted_when_*` check to any of the three vent-floor branches, choose ordering already guarantees the earlier lockout branch claims the confirmed+flag-on case first.
